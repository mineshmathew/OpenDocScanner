...
public class CroppingTrapezoid {
    ...
    public CroppingTrapezoid(ArrayList<Point> cropRect, Rect imageRect) {
...
// below part added by minesh, on 17/03/2016
        mImageRect = new Rect(imageRect);
        mPoints[0] = (float) cropRect.get(0).x;
        mPoints[1] = (float) cropRect.get(0).y;
        mPoints[2] = (float) cropRect.get(3).x;
        mPoints[3] = (float) cropRect.get(3).y;
        mPoints[4] = (float) cropRect.get(2).x;
        mPoints[5] = (float) cropRect.get(2).y;
        mPoints[6] = (float) cropRect.get(1).x;
        mPoints[7] = (float) cropRect.get(1).y;


        Log.w("myApp", "mPoints from constr of trpzd points are");
        Log.w("myApp", "p0 " + mPoints[0]);
        Log.w("myApp", "p1 " + mPoints[1]);
        Log.w("myApp", "p2 " + mPoints[2]);
        Log.w("myApp", "p3 " + mPoints[3]);
        Log.w("myApp", "p4 " + mPoints[4]);
        Log.w("myApp", "p5 " + mPoints[5]);
        Log.w("myApp", "p6 " + mPoints[6]);
        Log.w("myApp", "p7 " + mPoints[7]);
    ...
    public float[] getScreenPoints(Matrix matrix) {
        ...
        //mappedpoints hold the screepoints based on which the trapezoid can be drawn
    ...
    public void moveBy(float dx, float dy) {
        ...
        capPoints(HighlightView.GROW_NONE);
    }
...
//        dy = capDy(dy, boundingRect);
        ...
        if ((HighlightView.GROW_LEFT_EDGE | HighlightView.GROW_TOP_EDGE) == edge) {
            mPoints[0] += dx;
            mPoints[1] += dy;
            capPoints(edge);
            return;
        }
        if ((HighlightView.GROW_RIGHT_EDGE | HighlightView.GROW_TOP_EDGE) == edge) {
            mPoints[2] += dx;
            mPoints[3] += dy;
            capPoints(edge);
            return;
        }
        if ((HighlightView.GROW_RIGHT_EDGE | HighlightView.GROW_BOTTOM_EDGE) == edge) {
            mPoints[4] += dx;
            mPoints[5] += dy;
            capPoints(edge);
            return;
        }
        if ((HighlightView.GROW_LEFT_EDGE | HighlightView.GROW_BOTTOM_EDGE) == edge) {
            mPoints[6] += dx;
            mPoints[7] += dy;
            capPoints(edge);
            return;
        }
        if ((HighlightView.GROW_LEFT_EDGE) == edge) {
            mPoints[0] += dx;
            mPoints[1] += dy;
            mPoints[6] += dx;
            mPoints[7] += dy;
        }
        if ((HighlightView.GROW_RIGHT_EDGE) == edge) {
            mPoints[2] += dx;
            mPoints[3] += dy;
            mPoints[4] += dx;
            mPoints[5] += dy;
        }
        if ((HighlightView.GROW_TOP_EDGE) == edge) {
            mPoints[0] += dx;
            mPoints[1] += dy;
            mPoints[2] += dx;
            mPoints[3] += dy;
        }
        if ((HighlightView.GROW_BOTTOM_EDGE) == edge) {
            mPoints[4] += dx;
            mPoints[5] += dy;
            mPoints[6] += dx;
            mPoints[7] += dy;
    ...
    public int getHit(float x, float y, float hysteresis) {
        ...
        if (touchesTopLeft) {
            ...
            return HighlightView.GROW_LEFT_EDGE | HighlightView.GROW_TOP_EDGE;
        }
        final boolean touchesTopRight = calculateDistanceToPoint(mPoints[2], mPoints[3], x, y) <= hysteresis;
        if (touchesTopRight) {
            Log.i(LOG_TAG, "top right");
            return HighlightView.GROW_RIGHT_EDGE | HighlightView.GROW_TOP_EDGE;
        }
        ...
        if (touchesBottomRight) {
            ...
            return HighlightView.GROW_RIGHT_EDGE | HighlightView.GROW_BOTTOM_EDGE;
        }
        ...
        if (touchesBottomLeft) {
            ...
            return HighlightView.GROW_LEFT_EDGE | HighlightView.GROW_BOTTOM_EDGE;
        }
        ...
        if (retval == HighlightView.GROW_NONE) {
